...
- **PR の CI 完了待ちが発生する場面では、待機を開始する前に「green になったら merge するか」を closed な選択式 (AskUserQuestion) で先に確認しておく** (2026-08-31 PR #211 で確立): CI 完了は数十秒〜数分かかり、その間に user が離席する可能性がある。待機後に改めて確認を挟むと無応答で止まりやすいので、待機前に merge 可否の承認を取っておけば、green 確認後は追加確認なしでそのまま merge まで完了できる。PR #211 (#208 対応) で実践し、承認取得後の待機 → 自動 merge までノンストップで完了した。既存「承認や選択は closed な選択式で聞く」方針の **CI 待機版**。
...
- **並列実装 agent への dispatch prompt にはファイル所有権を明示分離して書く** (Wave1 2026-07-12 で確立): 複数 issue を worktree 分離で並列実装させる場合、各 agent の prompt に「スコープ外（他 agent が並行作業中 — 触らないこと）」節を置き、共有ファイル (ContentView / SettingsView 等) は行・領域単位で担当を割る。境界上の 1 行だけ触る場合も「:41 の 1 行のみ」と限定する。Wave1 (5 PR 同時) はこれで conflict ゼロで着地した。
- **並列 PR の全 merge 後に main で統合テストを必ず回す**: 各 PR のテストは「merge 前 main + 自 PR」に対する green でしかなく、PR 同士の意味的衝突 (semantic conflict) は誰も検証していない。全 merge 後の main で `xcodebuild test`（最低 unit 全体）を回してから完了と判定する。
- **`gh pr merge --delete-branch` は agent worktree が branch を掴んでいると local branch 削除で fail する**: merge 自体は成功しているので `gh pr view --json state` で MERGED を確認すれば OK。後始末は `git worktree remove <path> --force` → `git worktree prune` → `git branch -D <branch>` の順。
- **タスクごとの spec + code-quality 二段レビュー (+ 最終総合レビュー) を「機械的確認」と侮らず必ず通す**: レビューは rubber-stamp ではなく品質向上の実質工程。#84 では plan に無かった i18n 漏れ・today/月境界/年境界テスト・a11y 改善 (記録ドットの VoiceOver hidden + locale 対応日付ラベル) を二段レビューが引き出し plan を超える品質になった。実テストや改善を足したら「ルール化」を忘れがちなので工程として明文化する。最終 whole-branch レビューも実バグを出す実績が継続: #147 では spec→plan 変換で落ちた進捗バー項目、#148 では rename 時に絵文字が ✨ 降格する WYSIWYG バグを検出した (いずれも per-task レビューは通過していた)。#155 PR #191 では code-quality レビューが「i18n helper 化で `groupedRecords` sort 内の `DateFormatter` 生成が 1 回 → O(n²) 回になる perf regression」を検出 (テスト green・spec 準拠でも出ない種類の欠陥。helper 抽出 refactor では「旧実装が 1 インスタンスを使い回していた箇所」のコスト特性まで diff で確認する)。
- **inline 実行 (executing-plans) でも最終 whole-branch レビューを必ず通す** (2026-08-15 #177 PR #198 で確立): 上記「二段レビュー + 最終総合レビュー」ルールは subagent-driven 前提で書かれているが、inline 実行でも省略しない。#198 では component テスト green・plan 準拠でも出ない視覚 regression 2 件 (テーマカラーグラデ上の絵文字コントラストが子テーマ色次第で無制限に低下 / AX サイズ Dynamic Type での絵文字 truncate) を whole-branch レビューが検出し merge 前に修正できた。視覚変更を「手動確認推奨」境界 (PR #192 パターン) で出す場合こそ、レビューが実質的な視覚ゲートになる。
- **実装 subagent への dispatch prompt に「xcodebuild は FOREGROUND 実行必須 (background 禁止)」を明記する** (2026-07-18 #147 で確立): subagent が `xcodebuild test` を background 実行すると、完了通知を待つ形で turn を終えてしまい、テスト結果未確認・commit 未作成のまま**停止**する (1 session で 2 回発生)。dispatch prompt に「every command FOREGROUND / `** TEST SUCCEEDED **` 文言判定 / exit code 不可」を明記したら以降の発生ゼロ。もし停止したら (a) `git status` + report ファイルで進捗を実地確認 → (b) `pgrep xcodebuild` で実行残がないことを確認 → (c) **SendMessage で同じ agent を再開**させて続行 (transcript が残っているので新規 dispatch より安い)。
- **reviewer / implementer subagent は最終レポートを書き終えても送信せず idle になることがある → idle 通知だけ来たら催促 1 通で回収する** (2026-08-23 PR #204 の SDD 実行で確立、4 dispatch 中 3 回発生): dispatch prompt 末尾に「完了したら SendMessage で to: "team-lead" へレポート全文を送信」を明記しても、レポートを自分の最終テキストに書いただけで送信せず idle になるケースがある。idle 通知のみでレポート未着なら、「完成済みレポートを SendMessage で今すぐ送信して」と催促すれば 1 通で回収できる (再 dispatch 不要)。なお session limit 切れで implementer が mid-task 停止した場合も、既存の (a) 実地確認 → (b) pgrep → (c) SendMessage 再開の手順がそのまま有効 (同 PR の Task 3 で実証)。
- **dispatch prompt に書く simulator destination は `xcrun simctl list devices available | grep iPhone` で実在確認してから書く** (2026-08-08 で確立): repo 内 script のデフォルト (`benchmark-tests.sh` / `prepare-simulator.sh` の `iPhone 16`) は Xcode 更新で silent に stale になる。stale なまま dispatch prompt へ転記すると、並列 agent 全員が同じ destination 不在 deviation を踏む (2026-08-08 に 2 agent で発生、iPhone 17 で各自代替)。stale を見つけたら script のデフォルトも同 session で更新する。
- **subagent-driven-development の開始時に前回 run の `.superpowers/sdd/` 残骸 (task-*-brief/report, review-*.diff) を必ず掃除する** (2026-07-18 で確立): 旧 run の `task-3-report.md` を現 run の report と誤読し「別 plan の Task 3」を review しかけた実例あり。ledger (progress.md) を新 plan 用に書き直すタイミングで `rm -f task-*-brief.md task-*-report.md review-*.diff` を同時に行う。report を読む際は**冒頭の見出しが現 plan のタスク名と一致するか**を必ず確認する。
...
- **「既修正 / 既決定」検出を「依存・ブロッカーの stale 検出」にも広げる** (2026-08-13 #154 で確立): issue 本文の「優先: 低（#NNN の後で OK）」「#NNN で解消見込み」といった **依存記述も古くなる**。triage で「依存待ち」を理由に候補から外す前に、(a) 依存先 issue/PR の state を `gh issue view <N> --json state` で確認し、(b) 依存の根拠として挙げられた**コード上の症状** (例「`@unchecked Sendable` が残っている」) を grep で実地確認する。#154 は「#142 の後で OK」と書かれていたが #142 は merge 済み・`@unchecked Sendable` は grep 0 件で、**ブロッカーは既に外れていた** (低優先のまま放置される寸前だった)。既存「git log で既修正検出」「決定ログで既決定検出」の **依存関係版**。逆向きの stale もある: #151 の「スプラッシュは #16 の刷新とセットで」は #16 が close 済みで**依存先を失っており**、その項目を扱う場所が宙に浮いていた。
...
- **削除対象の「参照 0」という issue 記載は外部参照と内部参照を区別して検証する**: issue 本文の「N 構造体が参照 0」は**ファイル跨ぎの外部参照**しか数えていないことがある。同一ファイル内の他要素からの**内部利用** (例: `SkeletonViews.swift` で 8 構造体中 5 つが `HomeViewSkeleton` から内部利用) を見逃すと削除でビルドが壊れる。削除前に対象ファイル内での相互参照も grep で確認する。上記「対応案を鵜呑みにしない」ルールの削除系バージョン。
- **`git mv` でファイルを移動したら inbound / outbound の両方向の参照を確認する**: 移動ファイルを**指す**参照 (inbound) の grep だけでは不十分で、移動ファイル**内から出る**相対パスリンク (outbound: `./docs/screenshots/...` 等の画像 / ドキュメント相対リンク) が静かに壊れる (`RELEASE_v1.1.1_ASC_EN.md` 移動で advisor 指摘により発覚した実例)。移動後に移動ファイル内の相対パスを grep して deep link を検証する。
- **ドキュメントを新ディレクトリへ移動する際は静的サイトの `exclude` 設定を確認する**: GitHub Pages 等の `docs/_config.yml` の `exclude` がパスベースの場合、移動によって非公開文書が除外から外れて**意図せず公開される**可能性がある。移動先が引き続き除外対象かを移動時の verification に含める。
...
- **`xcodebuild test` の結果判定は `| tail -N` ではなく `grep -E '\*\* TEST'` で判定行を直接抽出する** (2026-07-20 PR #179 検証時に確立): unit 全体のような大きい suite では、成功時も末尾がテストケース行 (`Test case ... passed`) で埋まり `tail -15` に `** TEST SUCCEEDED **` が入らないことがある。判定行が見えず再実行する無駄 (フルスイート 1 回分) が出るので、最初から `xcodebuild test ... 2>&1 | grep -E "\*\* TEST|Failing tests" | tail -5` の形で判定文言を抽出する。個別 `-only-testing:` のような小さい実行では `tail -20` でも届くが、統一しておくと安全。**`-A6` を必ず付ける** (2026-08-08 PR #189 検証時に補強): 失敗テスト名は `Failing tests:` の**次行以降**に出るため、`-A` 無しの grep では「FAILED とだけ分かって名前が取れず、名前を取るためだけにフルスイートをもう 1 回回す」無駄が出る (実際に発生)。推奨形: `xcodebuild test ... > "$LOG" 2>&1; grep -E -A6 '\*\* TEST|Failing tests:' "$LOG"` — ログをファイル保存しておけば再実行なしで任意の詳細を掘れる。
...
- **ビルド設定変更 (warning 系フラグ等) の影響計測は、専用 `-derivedDataPath` の clean build で行う** (2026-08-13 #154 PR #197 で確立): `SWIFT_STRICT_CONCURRENCY=targeted` の影響を測ろうと通常の `xcodebuild build` を流したら **warning 1 件 (無関係な AppIntents note のみ)** で「影響ゼロ」と誤判定しかけた。直前のテスト実行でモジュールがキャッシュ済みだと Swift ファイルが再コンパイルされず、**新しい警告フラグが一切適用されない**。正しい手順: (a) `xcodebuild build-for-testing ... -derivedDataPath <scratch>/dd-<条件名>` で条件ごとに**まっさらな DerivedData** を使い (テストターゲットまで含めて計測できる)、(b) 設定なしの baseline も同じ方法で取り、(c) `comm -13 <(sort baseline) <(sort after)` で**差分だけ**を抽出する。実測では baseline 16 件 → targeted 25 件 (新規 9 件 = 実質 3 箇所) と判明した。pbxproj へ書いた設定が全ターゲットへ効いているかは `xcodebuild -target <各ターゲット> -showBuildSettings | grep <設定名>` で個別確認する (project レベルに 1 度書けば継承されるが、確認は 3 ターゲット分行う)。
...
- **既存条件を「厳格化」するテスト変更では red の代替に mutation 検証を行う** (PR #180 / PR #198 b687c12 で 2 回適用済みの反復パターン、2026-08-15 確立): OR→AND のような判定強化では既存実装が既に両条件を満たしていることが多く、red が原理的に発生しない。この場合は red 確認を省略してよいが、代わりに **対象条件の片方を意図的に破壊してテストが FAIL することを確認 → revert して GREEN 復帰** する mutation 検証を実施し、強化した両条件が実際に検出力を持つことを証明する。commit メッセージに「mutation 検証を TDD red の代替とする」旨を明記して reviewer が追えるようにする。
- **writing-plans で spec の列挙項目 (置換対象リスト・画面別変更点等) を plan task へ変換したら、spec 項目 → plan step の 1:1 対応を最後に指差し確認する**: spec の行参照が複数項目で重なっていると plan 変換時に項目が silent に脱落する。実例: #147 で spec §4 の「進捗バー(:170): brandPrimary」が、plan が同じ :170 を CTA ボタンと解釈したため脱落し、最終レビューまで誰も気付かなかった (fix 2 行で済んだが plan 段階なら無コスト)。spec 側も「行番号だけでなく要素名 (進捗バー / CTA ボタン) で項目を特定する」書き方にすると衝突自体を防げる。
...
- **Tutorial / sheet など「初回状態でしか出ない UI」の視覚検証は、UserDefaults launch args 上書き + 一時 UITest で行う** (2026-08-08 PR #188/#189 で確立): `--uitesting` は tutorial を skip してしまうため使わず、一時 UITest の `launchArguments` に `["-hasLaunchedBefore", "NO", "-hasCompletedChildTutorial", "NO"]` の形で **UserDefaults キーを per-launch 上書き**して tutorial を確実に表示させる (`-key value` 形式の launch args は `UserDefaults.standard` を上書きする iOS 標準機構、simulator の erase 不要で決定的)。UITest 内なら `app.buttons["次へ"].tap()` で step 遷移や sheet 表示もでき、simctl の tap 不可制約を回避できる。before/after 比較は「一時 UITest (untracked) を残したまま `git stash push <対象ファイル>` → 撮影 → `git stash pop`」で同一テストを新旧コードに対して流す。一時 UITest は目視後に削除して commit しない (#148 の swipeUp パターンと同じ discard 運用)。
- **02-record の initial view はカレンダーが fold を占有し、タスクカード等の下部 UI は写らない → fold 下の視覚検証は UITest への一時 swipeUp で行う** (2026-07-18 #148 で確立): `ASCScreenshotUITests` の record タブ tap と `attach` の間に `app.swipeUp(); sleep(1)` を **working tree のみの一時変更**として挿入 → `capture-asc-screenshots.sh` 実行 → 目視 → `git checkout -- app/OtetsudaiCoinUITests docs/screenshots/` で TEMP 変更とスクショを両方 discard する (commit しない)。fold 下 UI の変更が続くなら scroll 版の検証専用ショット (ASC 非提出) の script 正式追加を検討 (#177 に記載)。
...
## SwiftUI @Observable 継承の落とし穴
- **`BaseViewModel` (@Observable) を継承するサブクラスには `@Observable` を必ず再付与する** (#145 PR #170 で実証): 再付与しないと**サブクラス側で新規宣言した stored property が Observation 追跡されず、SwiftUI View が再描画されない**（旧 `ChildManagementViewModel.children` で潜在していた実バグ）。基底クラス宣言のプロパティ (`isLoading` 等) は付与有無に関わらず追跡されるため、通常の unit test（値の読み書き assert）では検知不能。検証は `withObservationTracking` を使った observation テストでのみ可能で、回帰ガードとして `ViewModelObservationTrackingTests` に恒久化済み。新規 ViewModel を BaseViewModel 継承で作るときは `@MainActor @Observable class Xxx: BaseViewModel` の形を必ず守る。
...
- **`findAll(Text/Shape)` は custom subview の境界を跨いで traverse できる** (2026-08-23 #200 PR #204 で実証、iOS 26 + ViewInspector 0.10.2): TaskCardView の taskIcon を `TaskIconView` component へ抽出した後も、親 View への `findAll(ViewType.Text.self)` / `findAll(ViewType.Shape.self)` は component 内部の Text / Shape まで到達し、既存の親テストが無変更で green を維持した (抽出直後に親テスト単独実行で実証ゲートを通す手順も同 PR で確立)。component 抽出 refactor の plan では「親テストが壊れる前提の fallback 設計」を必須にしなくてよいが、**抽出直後に親テストを単独実行して実証ゲートを踏む**手順は残す (ViewInspector のバージョン更新で挙動が変わりうるため)。
- **絵文字 / 可変サイズ Text のアイコンは固定 frame + `.overlay` / `.background` に入れず、ZStack + 別 frame の図形 + 非拘束 Text で組む** (2026-08-15 #177 PR #198 review で確立): `Text` は width 提案に従って truncate する (旧 `Image(systemName:)` は overflow するだけで消えなかった) ため、`.font(.title3)` 等のスケーリングフォントを固定 44pt 円の overlay や 32pt frame + background に入れると AX サイズの Dynamic Type で絵文字が切れる。TaskCardView の taskIcon (`ZStack { Circle().frame(...); Text(...).accessibilityHidden(true) }`) が参照パターンで、#198 で HelpRecordRow / TaskSelectionRow を同型へ統一済み。列揃え目的の固定幅は `.frame(width:)` でなく `.frame(minWidth:)` にする (TaskRowView)。
...
- **XCUITest で `Toggle` を操作するときは「行」ではなく「スイッチ本体」を叩く** (2026-07-28 PR #183 で確立、#182 と合わせ**2 セッション連続で踏んだ**): `app.switches` は 1 つの `Toggle` につき **2 要素**を返す。(a) ラベル付きの**行**全体 (例 `label='効果音'`, `frame=(20, 746, 400, 52)`)、(b) ラベル無しの**スイッチ本体** (`frame=(339, 758, 63, 28)` = 行の右端)。`XCUIElement.tap()` は要素の**中心**を叩くため、`app.switches.element(boundBy: 0).tap()` は x≈220 の「ラベル文字の上」を押すことになり **List 上では値が変わらない**。`exists`/`isHittable` は true、`tap()` も throw しないので、**「タップは届いたのに値が変わらない」= 実装バグ**に見える。対処: 値の読み取りは行要素 (`element(boundBy:)` で locale 非依存) のまま、**タップだけ `row.coordinate(withNormalizedOffset: CGVector(dx: 0.92, dy: 0.5)).tap()` で右端を狙う**。`element(boundBy: 2)` でスイッチ本体を直接掴む手もあるが、ツリーの形に依存するので座標指定の方が壊れにくい。**診断の型**: 症状が同じなので、原因を推測で decide せず `app.switches.count` と各要素の `label` / `frame` / `isHittable` を dump して切り分ける (print は xcodebuild の stdout に出ないので、**診断値を意図的な `XCTFail` のメッセージに載せて `xcrun xcresulttool get test-results tests` で回収**する)。PR #182 はこの罠でトグルの検証を諦め「⚠️ 未検証」のまま merge し、次セッションでも同じ罠で一度「実バグ」と誤判定した (実装は正常だった)。
- **`@Environment(\.isEnabled)` など環境値駆動の見た目分岐は判定を static pure helper へ切り出す** (2026-08-08 #175 PR #189 で確立): ViewInspector は環境値を注入できないため、`@Environment(\.isEnabled)` を読んで色を決める `ButtonStyle` の分岐はそのままではテスト不能。上記 #125 の gesture pure-関数抽出と同じ要領で、判定部分を `SolidButtonStyle.background(for:isEnabled:)` (`Presentation/Views/Components/AppButtonStyle.swift:36`) のような static pure helper へ抽出し、**環境値の配線だけを untested に残す**のが正しい境界。「環境値だからテスト範囲外」と即断しない。
...
- **Age Rating の「広告」=「はい」設定が必須** (Issue #86, v1.1.2 提出時 2026-05-23): AdMob (Google Mobile Ads) を統合した v1.1.0 以降は、ASC → App 情報 → 年齢制限指定で「広告」を必ず「はい」に設定する。No のままだと automated review が SDK 統合を検知して reject する。metadata-only fix なので build 再アップロードは不要、ASC UI で切替えるのみ。リリース手順書 (`docs/releases/RELEASE_vX.Y.Z.md`) の提出前チェックリストに項目化済み。
- **英語ロケーションの Description / What's New には絵文字を入れない** (Issue #85, 2026-05-23): ASC は en locale の text フィールドの絵文字を弾く (公式ドキュメント未明記の挙動)。日本語ロケーションでは絵文字 OK (v1.1.1 ja What's New で ✨🐛🌍 を公開実績あり)。en draft は plain text 見出し + `- ` ハイフン箇条書きで統一する。`docs/releases/RELEASE_vX.Y.Z.md` § What's New (en) / `docs/releases/RELEASE_vX.Y.Z_ASC_EN.md` 系の draft section に "plain text only, no emojis" 注意書きを残す運用とする。
...
- **`release-version-bump-check` CI gate の射程外**: `scripts/version-bump-check.sh` + `version-bump-check.yml` は **PR 経由で pbxproj が変更される release flow** のみを gating する。**Xcode から直接 Archive → Distribute する経路には介在できない**。よってリリース手順書 (`docs/releases/RELEASE_vX.Y.Z.md`) § 1.2 の build 選択前のチェックリストに「**`git tag --list 'v*' --sort=-v:refname | head -1` または ASC の最新 approved version より pbxproj の `MARKETING_VERSION` が高いことを再確認**」を必ず置く。CI に頼り切らず、ローカル archive flow にもガードを残す。
- **ASC で approved になったら速やかに `git tag -a vX.Y.Z` を push**: v1.1.2 リリースで git tag が打たれず、release-version-bump-check.yml の base ref (`git describe --tags --abbrev=0`) が v1.1.1 のままになっていた。tag が無いと CI が「approved 済み版」と pbxproj 差分を比較できなくなる。`docs/releases/RELEASE_vX.Y.Z.md` § 5 完了後タスクに既に項目化済みだが、approved 通知を受けた段階で **即タグ push** する運用を徹底する (リリース時に Internal/External Release Notes を整える前に tag は先打ち)。
